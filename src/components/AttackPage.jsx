...
export default class AttackPage extends PokemanPage {
  ...
  generateEnergyInfo (energy, key) {
    ...
    return <div className={'attack-page-attack-nrg' + percent + ' POKEMON_TYPE_' + key} />
  }
  ...
  render () {
          ...
          <div className='attack-page-container'>
            <div className='navbar'>
              ...
              <div className='right-panel'>
                <i className='fa fa-search fa-lg search-icon' aria-hidden='true' onClick={() => this.onActivateSearch(true)} />
              </div>
            </div>
          </div>
        )
    ...
    if (showFilters) {
      parts.push(
        <div>
          <TypeFilter isMono types={this.state.types} onTypeClicked={(list) => this.onTypeClicked(list)} />
