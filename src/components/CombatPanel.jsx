...
class CombatPanel extends React.Component {
  ...
  generateInfoTable (pokemon) {
    ...
    rows.push(<tr><td colSpan='2' align='left'>Distance copain</td><td align='right'>{ pokemon.buddy + ' KM' }</td></tr>)
  ...
  generateShinyButton (pokemon) {
    const hasShiny = PokedexHelper.hasShiny(pokemon.id)
    if (!hasShiny) {
      return null
    }
