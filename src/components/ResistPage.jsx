...
export default class ResistPage extends PokemanPage {
  constructor () {
    ...
    let error = true; let mode = ResistPage.MODE.ShowPokemon

    let pokemon = this.getPokemon(); let species = []
    if (pokemon) {
      ...
      if (ids) {
        let parts = ids.split(','); let speciesText = []
        for (let speciesId of parts) {
          let id = Number(speciesId); let speciesInfo = PokedexHelper.getSpecies(id)
  ...
  generateNoBonuses (data) {
    ...
    return (
      <div>
        <i className='fa fa-info-circle fa-pokeman-normal resist-info-icon' aria-hidden='true' />
  ...
  buildRows (data, isAttack) {
    let strength = []; let weak = []; let noBonuses = []
