...
class PokedexHelper {
  ...
  getImagePath (pokemon, isShiny) {
    let path = '../assets/pokemons/'; let id = pokemon.id
    if (isShiny) {
      path += 'shiny/'
  ...
  getAttacks (pokemonId) {
    let pokemon = this.getPokemon(pokemonId); let fast = []; let charged = []
    if (pokemon.attacks) {
  ...
  getDamageStrengthWeakness (damageType) {
    let species = this.getSpecies(damageType); let strength = {}
    Object.keys(species.dmg).forEach(id => strength[id] = species.dmg[id])

    let damageTypes = this.species(); let weakness = {}
  ...
  search (query) {
    let ids = []; let text = query.text.trim()
    if (text.length) {
